...
export const buildCovaraSchema = (
...
export const generateTypeScriptTypes = (schema: CovaraSchema): string => {
  let output = `// Generated by Covara v${schema.version}\n`;
  ...
  output += `export interface CovaraClient {\n`;
  for (const resource of schema.resources) {
...
export interface CovaraRouterConfig extends OpenAPIConfig {
  pathPrefix?: string;
}

export function createCovaraRouter(config?: CovaraRouterConfig): Hono;
export function createCovaraRouter(resources: RegisteredResource[], openApiConfig?: OpenAPIConfig): Hono;
export function createCovaraRouter(
  resourcesOrConfig?: RegisteredResource[] | CovaraRouterConfig,
  ...
  const config: CovaraRouterConfig = isAutoDiscovery
    ? (resourcesOrConfig as CovaraRouterConfig) ?? {}
    : openApiConfig ?? {};
...
export const SCHEMA_ENDPOINT = "/__covara";
